...
static int xcoder_logan_setup_encoder(AVCodecContext *avctx)
{
  ...
  {
    // In the vfr mode, customer may reset time base to a very large value, such
    // as 1000. At this time, the calculated framerate that depends on timebase
    // and ticks_per_frame may be incorrect. If the calculated framerate is
    // between 5 - 120, assume it is correct and keep it. Else, set to a default
    // of 30fps.
