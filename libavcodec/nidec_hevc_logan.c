...
static const AVOption dec_options[] = {
  ...
  { "ni_dec_idx",       "Select which decoder to use by index. First is 0, second is 1, and so on.", OFFSETDEC(dev_dec_idx),
    AV_OPT_TYPE_INT, {.i64 = NI_LOGAN_INVALID_HWID }, -1, INT_MAX, VD, "ni_dec_idx" },

  { "ni_dec_name", "Select which decoder to use by index. First is /dev/nvme0n1, second is /dev/nvme0n2, and so on.", OFFSETDEC(dev_dec_name),
    AV_OPT_TYPE_STRING, { .str = "" }, 0, 0, VD, "ni_dec_name" },
