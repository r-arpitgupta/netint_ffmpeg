...
while [ "$1" != "" ]; do
    case $1 in
        -h | --help) echo "Usage: ./build_ffmpeg.sh --quadra | --logan [OPTION]";
                     ...
                     echo "--nidrawtext                  build for ni_quadra_drawtext filter on Linux (requires libfreetype, libharfbuzz; recommend libfontconfig and libfribidi).";
...
if [[ $target_windows == "false" && $target_android == "false" && $enable_ni_drawtext == "true" ]]; then
    extra_config_flags="${extra_config_flags} --enable-libfreetype --enable-libfontconfig --enable-libharfbuzz --enable-pic"
else
