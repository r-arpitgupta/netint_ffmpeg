...
int avformat_find_stream_info(AVFormatContext *ic, AVDictionary **options)
{
    ...
    for (unsigned i = 0; i < ic->nb_streams; i++) {
        ...
        if (avctx->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (!st->r_frame_rate.num) {
                const AVCodecDescriptor *desc = sti->codec_desc;
                // NETINT avctx->framerate is already the correct value, at
                // least for avc, so don't adjust (x2); do this for avc for now
                // and watching for any side effect
                // AVRational mul = (AVRational){ desc && (desc->props & AV_CODEC_PROP_FIELDS) ? 2 : 1, 1 };
                AVRational mul = (AVRational){ desc && (desc->props & AV_CODEC_PROP_FIELDS) && avctx->codec_id != AV_CODEC_ID_H264 ? 2 : 1, 1 };
                ...
                av_log(avctx, AV_LOG_VERBOSE, "%s st->r_frame_rate %d/%d\n", __func__, st->r_frame_rate.num, st->r_frame_rate.den);
            }
