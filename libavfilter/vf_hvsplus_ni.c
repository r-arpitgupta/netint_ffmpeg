...
static int init_ai_context(AVFilterContext *ctx, NetIntAiPreprocessContext *s,
   ...
   // Configure NB file
   av_log(ctx, AV_LOG_DEBUG, "%s: out w %d h %d NB w %d h %d\n", __func__,
          s->out_width, s->out_height, s->nb_width, s->nb_height);
   ...
   ai_ctx->api_ctx.pixel_format = ff_ni_ffmpeg_to_libxcoder_pix_fmt(frame->format);
...
static int ni_hvsplus_config_input(AVFilterContext *ctx, AVFrame *frame) {
   ...
   if (ret != 0) {
    ...
    if (s->channel_mode == 0) {
        av_log(ctx, AV_LOG_DEBUG, "%s: net %dx%d frame %dx%d\n", __func__, s->network.netw, s->network.neth, frame->width, frame->height);
        ...
        av_log(ctx, AV_LOG_DEBUG, "%s: net_out %dx%d frame %dx%d\n", __func__, s->network.net_out_w, s->network.net_out_h, s->out_width, s->out_height);
...
static int ni_hvsplus_output_config_props(AVFilterLink *outlink) {
    ...
    s->nb_width = (retval == 0) ? inlink->w : nbSizes[retval-1].width;
    s->nb_height = (retval == 0) ? inlink->h : nbSizes[retval-1].height;

    av_log(ctx, AV_LOG_INFO, "%s: inlink w %d h %d NB w %d h %d\n", __func__, inlink->w, inlink->h, s->nb_width, s->nb_height);

    // Disable HW frame processing for now.
    // Current implementation can support HW frame if ((outlink->w == s->nb_width) && (outlink->h == s->nb_height))
#if IS_FFMPEG_71_AND_ABOVE
    if(li->hw_frames_ctx != NULL) {
        av_log(ctx, AV_LOG_ERROR, "Error: HW frame not supported for hvsplus filter\n");
        return AVERROR(EINVAL);
    }
#else
    if(inlink->hw_frames_ctx != NULL) {
...
static int av_to_niframe_copy(ni_frame_t *dst, const AVFrame *src, int nb_planes) {
    ...
    av_log(NULL, AV_LOG_INFO, "%s: src width %d height %d format %s\n", __func__,
...
static int ni_to_avframe_copy(AVFrame *dst, const ni_packet_t *src, int nb_planes) {
    ...
    av_log(NULL, AV_LOG_INFO, "%s: dst width %d height %d format %s\n", __func__,
...
static int ni_hvsplus_filter_frame_internal(AVFilterLink *link, AVFrame *in) {
    ...
    av_log(ctx, AV_LOG_INFO, "%s: filter %p hwframe %d format %s\n", __func__, s, hwframe, av_get_pix_fmt_name(in->format));
    ...
    av_log(ctx, AV_LOG_INFO, "%s: out_width %d out_height %d\n", __func__, s->out_width, s->out_height);
    ...
    if (hwframe)
    {
        ...
        if (!out->data[3])
        {
            av_log(ctx, AV_LOG_ERROR, "ni hvsplus filter av_alloc returned NULL\n");
            ret = AVERROR(ENOMEM);
            goto failed_out;
        }
        ...
        do{
            ...
            if(retval < NI_RETCODE_SUCCESS){
                av_log(ctx, AV_LOG_ERROR, "failed to read hwdesc,retval=%d\n", retval);
                ret = AVERROR(EINVAL);
                goto failed_out;
            }
            if(av_gettime() - start_t > s->ai_timeout * 1000000){
        ...
        if (!out->buf[0])
        {
            av_log(ctx, AV_LOG_ERROR, "ni hvsplus filter av_buffer_create returned NULL\n");
            ret = AVERROR(ENOMEM);
    ...
    else
    {
        ...
        av_log(ctx, AV_LOG_INFO, "%s: format %d allocate frame %d x %d\n", __func__, in->format, out->width, out->height);
        ...
        if (retval < 0)
        {
            av_log(ctx, AV_LOG_ERROR, "hvsplus cannot copy frame\n");
            ret = AVERROR(EIO);
            goto failed_out;
        }
        ...
        if (retval < 0)
        {
            av_log(ctx, AV_LOG_ERROR, "hvsplus cannot copy ai frame to avframe\n");
            ret = AVERROR(EIO);
            goto failed_out;
        }
    }
...
static const AVOption ni_hvsplus_options[] = {
    ...
    {"keep_alive_timeout", "Specify a custom session keep alive timeout in seconds.", OFFSET(keep_alive_timeout),
      AV_OPT_TYPE_INT, {.i64 = NI_AI_PRE_KEEPALIVE_TIMEOUT}, NI_MIN_KEEP_ALIVE_TIMEOUT, NI_MAX_KEEP_ALIVE_TIMEOUT, FLAGS, "keep_alive_timeout"},
