                                        ...
                                        InputFilter *ifilter, AVFilterInOut *in)
{
    ...
    if ((ifp->opts.flags & IFILTER_FLAG_AUTOROTATE) &&
        ...
        if (ifp->format == AV_PIX_FMT_NI_QUAD)
        {
    ...
    for (int i = 0; i < fg->nb_inputs; i++) {
        ...
        while (av_fifo_read(ifp->frame_queue, &tmp, 1) >= 0) {
            if (ifp->type_src == AVMEDIA_TYPE_SUBTITLE) {
                ...
                // NETINT: fix timing info of the first src frame of filter
                tmp->pts       = av_rescale_q(tmp->pts,      tmp->time_base, ifp->time_base);
                tmp->duration  = av_rescale_q(tmp->duration, tmp->time_base, ifp->time_base);
                tmp->time_base = ifp->time_base;
#if LIBAVUTIL_VERSION_MAJOR < 59
                AV_NOWARN_DEPRECATED(
                tmp->pkt_duration = tmp->duration;
                )
#endif
