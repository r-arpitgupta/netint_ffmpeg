...
int enc_open(void *opaque, const AVFrame *frame)
{
    ...
    if (frame) {
        ...
        for (int i = 0; i < frame->nb_side_data; i++) {
            ...
            // NETINT Check desc to avoid coredump
    ...
    case AVMEDIA_TYPE_VIDEO: {
        ...
        // NETINT: Fix color info will be overwrite when set params in encoder
        if (enc_ctx->color_range == AVCOL_RANGE_UNSPECIFIED)
            enc_ctx->color_range            = frame->color_range;
        if (enc_ctx->color_primaries == AVCOL_PRI_UNSPECIFIED)
            enc_ctx->color_primaries        = frame->color_primaries;
        if (enc_ctx->color_trc == AVCOL_TRC_UNSPECIFIED)
            enc_ctx->color_trc              = frame->color_trc;
        if (enc_ctx->colorspace == AVCOL_SPC_UNSPECIFIED)
