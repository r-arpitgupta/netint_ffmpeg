                                     ...
                                     AVPacket *pkt)
{
    ...
    // NETINT: also include subtitle to prevent UDP buffer overrun at pts 33-bit wrap-around
...
static int ist_add(const OptionsContext *o, Demuxer *d, AVStream *st, AVDictionary **opts_used)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if ((st->avg_frame_rate.num != 0) && (st->avg_frame_rate.den != 0)) {
            ds->dec_opts.framerate = st->avg_frame_rate;
        } else if ((st->r_frame_rate.num != 0) && (st->r_frame_rate.den != 0)) {
            ds->dec_opts.framerate = st->r_frame_rate;
        }else {
            av_log(NULL, AV_LOG_WARNING, "No framerate info found or probed -> using a default 30 fps\n");
            ds->dec_opts.framerate.num = 30;
        }
