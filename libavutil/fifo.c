...
{
    return av_fifo_alloc2(size, 1, 0);
}
...
{
    return av_fifo_can_write(f)*av_fifo_elem_size(f);
}

int av_fifo_realloc2(AVFifo* f, unsigned int new_size)
{
    return av_fifo_grow2(f, new_size);
}

int av_fifo_generic_write(AVFifo* f, void* src, int size,
                          int (*func)(void*, void*, int))
{
    size_t nb_elemets = size;
    if (func)
    {
        return av_fifo_write_from_cb(f, func, src, &nb_elemets);
    }
    else
    {
        return av_fifo_write(f, src, nb_elemets);
...
{
    size_t nb_elemets = buf_size;
    return fifo_peek_common(f, dest, &nb_elemets,
                            0, func, dest);
}
